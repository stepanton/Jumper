...
namespace mygame
{
    public partial class mygame : Form
    {
        ...
        private void MainTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                canvas.Graphics.DrawImage(menu.BG,this.ClientRectangle);
            }
            else if (state == GameState.play)
            {
                
                if (direction == Direction.Right) game.InputProcessor(Keys.Space, state);
        ...
        private void mygame_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyData)
            {
                ...
                case Keys.Space:
                    {
                        if (state == GameState.start) state = GameState.play; break;
                        if (state == GameState.play) direction = Direction.Right; break;
                        if (state == GameState.pause) state = GameState.play; break;
                        
                    }
               //case Keys.Up: if (direction == Direction.Nothing) direction = Direction.Up; break;  
